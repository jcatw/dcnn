...
class NodeClassificationDCNN(object):
    def __init__(self, parameters, A):
        ...
        self.apply_loss_and_update = theano.function([self.var_K, self.var_X, self.var_Y], loss, updates=updates)
        self.apply_loss = theano.function([self.var_K, self.var_X, self.var_Y], loss)
    ...
    def _register_model_layers(self):
        ...
        for i in range(self.params.num_dcnn_layers):
            ...
            features_layer = lasagne.layers.ReshapeLayer(
                l_dcnn,
                (-1, (self.params.num_hops + 1) * self.params.num_features)
            )
            num_features = (self.params.num_hops + 1) * self.params.num_features
